...
ld formula(ll x1,ll y1,ll x2,ll y2){
...
int main()
{
    ...
    cin>>n;
    vpll pts(n);
    for0(i,0,n) cin>>pts[i].first>>pts[i].second;

    for0(i,0,n){
        ld maxi = 0.0;
        ll id;
        for0(j,0,n){
            if(formula(pts[i].first,pts[i].second,pts[j].first,pts[j].second)>maxi){
                maxi = formula(pts[i].first,pts[i].second,pts[j].first,pts[j].second);
                id = j+1;
            }
        }
        cout<<id<<"\n";
