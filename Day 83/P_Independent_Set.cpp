...
ll dfs(vvll &adj, ll node, ll par, ll prev, vvll &dp)
{
    ...
    if (prev == 1)
    {
        ...
        for (auto &child : adj[node])
        {
            if (child != par)
            {
                resw = ((resw % MOD) * (dfs(adj, child, node, 0, dp) % MOD)) % MOD;
            }
        }
    }
    else
    {
        for (auto &child : adj[node])
        {
            ...
            {
                resw = ((resw % MOD) * (dfs(adj, child, node, 0, dp) % MOD)) % MOD;
                resb = ((resb % MOD) * (dfs(adj, child, node, 1, dp) % MOD)) % MOD;
            }
        }
    }

    return dp[node][prev] = (resw % MOD + resb % MOD) % MOD;
}
int main()
{
    fastio;
