...
bool dfs(vvpll &adj, ll node, vbool &vis, vll &numLine){
    ...
    for(auto &child: adj[node]){
        ...
        ll d = child.second;
        if(!vis[v]){
            numLine[v] = numLine[node]-d;
            isPossible = dfs(adj,v,vis,numLine);
        }else{
            if(numLine[v]==numLine[node]-d) continue;
            else{
                isPossible = false;
            }
        }
...
int main()
{
    ...
    Tc
    {
        ...
        for1(i,1,n){
            if(!vis[i]){
                numLine[i] = 0;
                isPossible = dfs(adj,i,vis,numLine);
            }
            if(!isPossible) break;
        }
