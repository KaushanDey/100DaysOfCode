...
int main()
{
    ...
    sgmnt.pb({s,i-1});
    // for0(i,0,sgmnt.size()){
    //     cout<<sgmnt[i].first<<" "<<sgmnt[i].second<<"\n";
    // }
    ll ans = 0;
    for0(j,0,sgmnt.size()){
        ll s = sgmnt[j].first;
    ...
    for(ll j = 0;j<sgmnt.size()-1;j++){
        ll si = sgmnt[j].first;
        ll fi = sgmnt[j].second;
        ll si1 = sgmnt[j+1].first;
        ll fi1 = sgmnt[j+1].second;
        if(si1-fi==1){
            if(a[si1+1]>a[fi]){
                ans = max(ans,fi1-si1+fi-si+1);
            }
            if(a[si1]>a[fi-1]){
                ans = max(ans,fi1-si1+fi-si+1);
            }
