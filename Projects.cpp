...
bool Compare(Proj a, Proj b){
    return (a.f<b.f);
}

ll Solve(vector<Proj> &a, ll ind, ll &n,ll prev, vector<ll> &dp){

    if(ind==n-1){
        if(a[ind].s>prev) return a[ind].p;
        return 0;
    }
    if(dp[ind]!=-1) return dp[ind];
    ll take = LLONG_MIN;
    if(a[ind].s>prev) take = a[ind].p + Solve(a,ind+1,n,a[ind].f,dp);
    ll notTake = Solve(a,ind+1,n,prev,dp);
