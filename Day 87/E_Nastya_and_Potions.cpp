...
ll Solve(vvll &adj, ll ind, ll &n, vbool &present, vll &c, vll &dp){

    if(present[ind]) return 0;
    if(adj[ind].size()==0) return c[ind];

    if(dp[ind]!=-1) return dp[ind];

    ll sum = 0;
    for0(i,0,adj[ind].size()){
        sum+=Solve(adj,adj[ind][i],n,present,c,dp);
    }

    return dp[ind] = min(c[ind],sum);
}
int main()
{
    fastio;
    Tc
    {
        ...
        for0(i,0,n){
            ll ans = Solve(adj,i,n,present,c,dp);
            cout<<ans<<" ";
        }
        cout<<"\n";
    }
